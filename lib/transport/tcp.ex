defmodule DBux.Transport.TCP do
  ...
  def do_connect(transport_proc) do
    if @debug,
      do:
        Logger.debug(
          "[DBux.Transport.TCP #{inspect(self())}] Do connect: #{inspect(transport_proc)}"
        )
  ...
  def do_disconnect(transport_proc) do
    if @debug,
      do:
        Logger.debug(
          "[DBux.Transport.TCP #{inspect(self())}] Do disconnect: #{inspect(transport_proc)}"
        )
  ...
  def do_send(transport_proc, data) when is_binary(data) do
    if @debug,
      do:
        Logger.debug(
          "[DBux.Transport.TCP #{inspect(self())}] Do send: #{inspect(transport_proc)}"
        )
  ...
  def connect(_, %{sock: sock} = state) when is_binary(sock) do
    ...
    if @debug,
      do:
        Logger.debug(
          "[DBux.Transport.TCP #{inspect(self())}] Connect: Connecting to socket #{sock}"
        )
    ...
    case :gen_tcp.connect(
      ...
      {:ok, sock} ->
        if @debug,
          do:
            Logger.debug(
              "[DBux.Transport.TCP #{inspect(self())}] Connect: Successfully connected to socket #{sock}"
            )
  ...
  def connect(_, %{host: host, port: port} = state) do
    ...
    if @debug,
      do:
        Logger.debug(
          "[DBux.Transport.TCP #{inspect(self())}] Connect: Connecting to #{host}:#{port}"
        )

    case :gen_tcp.connect(
           to_char_list(host),
           port,
           [active: true, mode: :binary, packet: :line, keepalive: true, nodelay: true],
           @connect_timeout
         ) do
      {:ok, sock} ->
        if @debug,
          do:
            Logger.debug(
              "[DBux.Transport.TCP #{inspect(self())}] Connect: Successfully connected to #{host}:#{port}"
            )
      ...
      {:error, reason} ->
        Logger.warn(
          "[DBux.Transport.TCP #{inspect(self())}] Connect: Failed to connect to #{host}:#{port}: #{inspect(reason)}"
        )
  ...
  def handle_call(:connect, _sender, %{sock: nil} = state) do
    if @debug,
      do:
        Logger.debug(
          "[DBux.Transport.TCP #{inspect(self())}] Handle call: Connect while disconnected"
        )
  ...
  def handle_call(:connect, _sender, state) do
    if @debug,
      do:
        Logger.debug(
          "[DBux.Transport.TCP #{inspect(self())}] Handle call: Connect while connected"
        )
  ...
  def handle_call(:disconnect, _sender, %{sock: nil} = state) do
    if @debug,
      do:
        Logger.debug(
          "[DBux.Transport.TCP #{inspect(self())}] Handle call: Disconnect while disconnected"
        )
  ...
  def handle_call(:disconnect, _sender, state) do
    if @debug,
      do:
        Logger.debug(
          "[DBux.Transport.TCP #{inspect(self())}] Handle call: Disconnect while connected"
        )
  ...
  def handle_call({:send, _}, _sender, %{sock: nil} = state) do
    if @debug,
      do:
        Logger.debug(
          "[DBux.Transport.TCP #{inspect(self())}] Handle call: Send while disconnected"
        )
  ...
  def handle_call({:send, data}, _sender, %{sock: sock} = state) do
    ...
    case :gen_tcp.send(sock, data) do
      ...
      {:error, reason} ->
        Logger.warn(
          "[DBux.Transport.TCP #{inspect(self())}] Failed to send data: #{inspect(reason)}"
        )
  ...
  def handle_call(:begin, _sender, %{sock: nil} = state) do
    if @debug,
      do:
        Logger.debug(
          "[DBux.Transport.TCP #{inspect(self())}] Handle call: Begin while disconnected"
        )
  ...
  def handle_info({:tcp, _sock, "OK " <> _}, %{state: :handshake, parent: parent} = state) do
    if @debug,
      do: Logger.debug("[DBux.Transport.TCP #{inspect(self())}] Authentication succeeded")
  ...
  @doc false
  def handle_info(
        {:tcp, _sock, "REJECTED " <> _, reason},
        %{state: :handshake, parent: parent} = state
      ) do
  ...
  def handle_info({:tcp, _, data}, %{state: :ready, parent: parent} = state) do
    if @debug,
      do: Logger.debug("[DBux.Transport.TCP #{inspect(self())}] TCP read: #{inspect(data)}")
